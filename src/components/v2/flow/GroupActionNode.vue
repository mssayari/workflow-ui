...
<template>
  <div class="workflow-node bg-gray-100 p-4 shadow-md rounded-lg border border-dashed border-gray-200 min-w-72 relative">
    <div class="flex justify-between items-start mb-2">
      <div>
        <h3 class="font-semibold">{{ data.action.name }}</h3>
        <span class="text-sm text-gray-600">Provider:
      {{ actionStore.getProviderById(data.action.type_id, data.action.provider_id).name }}</span>
      </div>
    ...
    <div class="py-5 flex gap-2">
      <div v-for="(action,index) in data.action.actions" :key="index"
        ...
        <div class="flex items-center gap-2 py-2">
          <button @click="workflowStore.openActionModal(action)"
                  class="p-1 bg-gray-100 hover:bg-gray-200 rounded text-sm flex items-center gap-1">
            <icon-edit class="h-4 w-4"/>
            <span>Edit</span>
          </button>
        ...
        <button v-if="!action.on_success" @click="workflowStore.openActionModal(data.action,action.id,true)"
