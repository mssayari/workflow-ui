...
<template>
  <div class="workflow-node flex flex-col items-center gap-2 trigger-node relative">
    <div class="bg-gray-700 text-white p-3 rounded-lg shadow-lg w-64">
      <h3 class="font-semibold mb-2">{{ data.trigger.trigger.app.name }}</h3>
      <p class="text-sm">{{ data.trigger.trigger.name }}</p>
      <div class="flex items-center gap-1 pt-2">
        <button @click="workflowStore.openActionModal(true, data.trigger)"
    ...
    <button v-if="!workflowStore.workflow.actions.length" @click="workflowStore.openActionModal(false)"
