...
<template>
  <div class="space-y-4">
    <template v-for="(field, index) in schema" :key="index">
      <div>
        <label v-if="field.type !== 'boolean'" class="block text-sm font-medium text-gray-700 mb-1">{{
            field.label
          }}</label>
        ...
        <div><span v-if="errors['config.'+ field.name]"
                   class="text-red-500 text-sm">{{ errors['config.' + field.name][0] }}</span></div>
